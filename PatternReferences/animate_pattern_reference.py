...
def load_series(path: str, marker_id: int = 1) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        if i_x is None or i_y is None or i_z is None:
            raise ValueError(f"Columns for marker {marker_id} not found. Expected {marker_id}_X,{marker_id}_Y,{marker_id}_Z")
...
def _ask_choice() -> str:
    ...
    while True:
        choice = input("Enter 1/2 (or 'c'/'z', 'q' to quit): ").strip().lower()
        if choice in ("1", "c", "circle"):
            return "circle"
        if choice in ("2", "z", "zz", "zigzag"):
            return "zigzag"
        if choice in ("q", "quit", "exit"):
            raise SystemExit(0)
        print("Invalid choice, please enter 1, 2, c, z, or q.")


def _ensure_pattern(kind: str, script_dir: str) -> str:
    assert kind in ("circle", "zigzag")
    filename = f"Pattern_{'Circle' if kind == 'circle' else 'Zigzag'}.csv"
    ...
    gen = os.path.join(script_dir, "create_pattern_reference.py")
    if os.path.isfile(gen):
        try:
            print(f"Generating {filename} via create_pattern_reference.py ...")
...
def main():
    parser = argparse.ArgumentParser(description="Animate 3D point with path and projection from pattern reference CSV")
    parser.add_argument("csv", nargs="?", default=None, help="Input CSV path or shortcut: 'circle' or 'zigzag' (default: ask)")
    ...
    if path is None or path.strip().lower() in ("circle", "c", "1", "zigzag", "z", "zz", "2"):
        if path is None or path.strip().lower() in ("circle", "c", "1"):
            kind = "circle" if path else _ask_choice()
            if path and kind != "circle":
                kind = "circle"
        elif path.strip().lower() in ("zigzag", "z", "zz", "2"):
            kind = "zigzag"
        else:
            kind = _ask_choice()
        path = _ensure_pattern(kind, script_dir)
    else:
    ...
    if args.xy_view:
        ...
        if args.elev is not None or args.azim is not None:
            ax.view_init(elev=args.elev if args.elev is not None else ax.elev, azim=args.azim if args.azim is not None else ax.azim)
        if args.lock_view:
            ax.set_autoscale_on(False)
