...
- Input: Raw CSV exports (with a `TRAJECTORIES` section) from your capture software, placed under `Exports/Raw_Data/`.
...
- `Step1_csv_formatter.py` -> `Exports/Raw_Data_Formatted/`
- `Step2_data_clean_recursive.py` -> `Exports/Raw_Data_Clean/`
- `Step2-5_references.py` -> `Exports/Reference/`
- `Step3_interpolate.py` -> `Exports/Raw_Data_Interpolated/`
- `Step4_trim.py` -> `Exports/Trimmed_Data/`
- `Step5_average_1M.py` -> `Exports/Averaged_Data_1M/`
...
- `Step10_musterverlauf_analysis.py` -> compares averaged XY-trajectories against pattern references; also aggregates and creates visuals. Writes
  - `Exports/PatternDeviation_Summary.csv`
  - per-cluster overlays `…/circle|zigzag/pattern_compare.png`
  - heatmap `Exports/PatternDeviation_Heatmap_nRMSE.png`
  - boxplots `Exports/PatternDeviation_Boxplot_byTask.png`
  - bar chart `Exports/PatternDeviation_Bar_Combined.png`
- `Step10_aggregate_percent_table.py` -> aggregates the summary into a cluster→percentage table; writes
  - `Exports/PatternDeviation_PercentTable.csv`
  - `Exports/PatternDeviation_PercentTable_detailed.csv`
  - `Exports/PatternDeviation_Totals.txt`
...
Directory with references and tools for pattern comparisons:
- `PatternReferences/` — pattern definitions and helpers used in Step 10
  - `Circle.csv`, `Zigzag.csv` (base shapes)
  - `create_pattern_reference.py` (builds `Pattern_Circle.csv`, `Pattern_Zigzag.csv`)
  - `animate_pattern_reference.py` (visual sanity-check/animation)
  - `Pattern_Circle.csv`, `Pattern_Zigzag.csv` (generated references)

Notes:
- `data_clean_v1.py` exists but is not part of the official pipeline described below.
- Zipped raw acquisitions under `Exports/Participant*/...` are not consumed directly by these steps; the pipeline expects CSVs under `Exports/Raw_Data/`.
...
- Filenames include both participant and experiment (e.g., `Participant3_circle_....csv`).
  - Participant extraction uses `(participant|probant)\d+`.
...
- Purpose: Converts heterogeneous raw CSVs (`,` delimiter, `cp1252` encoding) into consistent `;`-separated files with rectangular shape under `Exports/Raw_Data_Formatted/`.
- Key details:
...
- Highlights:
  ...
  - Outputs to `Exports/Raw_Data_Clean/`.
...
- Output: `Exports/Raw_Data_Interpolated/` and a report of remaining NaNs.
...
- Output: `Exports/Trimmed_Data/`.

6) Step 5 — Average Trials Per Participant and Experiment
- File: `Step5_average_1M.py`
- Purpose: For each `(participant, experiment)` group, resamples each trimmed trial to the average frame count, then takes the mean and std across trials.
- Notes:
...
- Output: `Exports/Averaged_Data_1M/*_{experiment}_mean.csv` and `*_std.csv`.

7) Step 6 — Average Across Participants (by Experiment)
- File: `Step6_average_by_experiment_1M.py`
- Purpose: Aggregates all `*_mean.csv` for a given experiment across participants, resamples to the average length, and outputs a single across-subject mean.
...
- Purpose: Applies Savitzky-Golay smoothing to the across-participant means, column-wise.
...
- Purpose: Builds demographic clusters (by gender, age-within-gender, and age only) using a participant metadata map; averages within each cluster and experiment and plots scalar kinematics.
- Output: `Exports/Clustered/<Mode>/<Cluster>/<Experiment>/{mean.csv, velocity_acc_scalar.png}`.

11) Step 10 — Pattern Analysis + Percent Table + Visuals
...
- Purpose: Compare averaged trajectories (XY only, time-agnostic) against predefined pattern references, produce a compact cluster→percentage report, and generate overview plots.
- Alignment:
...
- Purpose: For `ptp`, `zigzag`, and `weight`, automatically select one “best” recording directly from `Exports/Raw_Data_Clean` (fewest gaps/extrapolation) and produce plots without averaging.
...
Assuming your raw CSVs are under `Exports/Raw_Data/`:
...
```
6. Average per participant/experiment
```
python Step5_average_1M.py
```
7. Average across participants (by experiment)
```
...
11. Pattern analysis + percent table + visuals
```
python Step10_musterverlauf_analysis.py
```
   - Uses references in `PatternReferences/`.
...
- Temporal Alignment: Resampling to the average length followed by a normalized percent timeline allows averaging across trials and participants without explicit time-warping.
...
- `Exports/Raw_Data_Formatted/`: formatted raw data (`;` delimited).
- `Exports/Raw_Data_Clean/`: parsed, numeric tables.
- `Exports/Reference/`: best reference trials for markers 4/5 (select experiments).
- `Exports/Raw_Data_Interpolated/`: interpolated and end-filled data.
- `Exports/Trimmed_Data/`: truncated sequences (movement only).
- `Exports/Averaged_Data_1M/`: per-participant means and stds.
...
- `Exports/PatternDeviation_Summary.csv`: per-cluster deviation metrics vs. pattern reference.
- `Exports/PatternDeviation_PercentTable.csv`: cluster → percentage (combined mean nRMSE%).
- `Exports/PatternDeviation_PercentTable_detailed.csv`: per-cluster circle/zigzag percentages.
- `Exports/PatternDeviation_Totals.txt`: overall totals.
- `…/circle|zigzag/pattern_compare.png`: overlay of averaged path vs. pattern reference in each cluster folder.
 - `Exports/PatternDeviation_Heatmap_nRMSE.png`: heatmap of nRMSE% by cluster × task.
 - `Exports/PatternDeviation_Boxplot_byTask.png`: boxplots of cluster nRMSE% grouped by task.
 - `Exports/PatternDeviation_Bar_Combined.png`: bar chart of combined mean nRMSE% per cluster.

## Re-running on New Data

1. Place new raw CSVs under `Exports/Raw_Data/`.
2. Run steps 1->9 as above.
3. Inspect outputs at each stage, adjust thresholds/windows if needed.

Tip: If file naming differs, update the experiment/participant detection rules in `Step5_average_1M.py` and `Step6_average_by_experiment_1M.py`.
