...
OUTPUT_ROOT = "Exports/Raw_Data_Interpolated"
...
def fill_trailing_with_neighbors(df: pd.DataFrame, target_id: int, neighbor_ids):
    for axis in AXES:
        ...
        best_col = max(neighbor_cols, key=lambda col: df[col].iloc[last_valid + 1 :].notna().sum(), default=None)
        if not best_col:
            continue
...
def fill_leading_with_neighbors(df: pd.DataFrame, target_id: int, neighbor_ids):
    for axis in AXES:
        ...
        best_col = max(neighbor_cols, key=lambda col: df[col].iloc[:first_valid].notna().sum(), default=None)
        if not best_col:
            continue
...
def report_remaining_nans(df: pd.DataFrame):
    ...
    if len(leftovers):
        print("[warn] Remaining NaNs:")
        for col, cnt in leftovers.items():
            print(f"  - {col}: {cnt}")
    else:
        print("[ok] All NaNs resolved.")


def detect_experiment_type(path: str) -> str | None:
    lowered = path.lower()
...
def interpolate_csv(input_path, output_path, ref_map):
    try:
        ...
        pd.concat([frames, filled], axis=1).to_csv(output_path, sep=DELIMITER, index=False, encoding="utf-8")
        print(f"[ok] Saved: {output_path}")
        report_remaining_nans(filled)

    except Exception as e:
        print(f"[error] Failed: {input_path} -> {e}")
